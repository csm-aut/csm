...
import os
import errno
from au.Log import Log
from au.utils.impl import format_exception
...
class Logfile(Log):
    ...
    def rename(self, filename):
        errorname = filename + '.error'
        if os.path.isfile(self.filename):
            try:
                os.rename(self.filename, filename)
            except OSError as e:
                print 'Error renaming log filename to %s: %s' % (filename, e)
                self.do_log = False
                raise
            self.filename = filename
        if os.path.isfile(self.errorname):
            try:
                os.rename(self.errorname, errorname)
            except OSError as e:
                print 'Error renaming log filename to %s: %s' % (errorname, e)
                self.do_log = False
                raise
